# scripts/04f_run_rspectra_operator.R
# ------------------------------------------------------------
# RSpectra eigendecomposition for OPERATOR THETA (Theta)
# RSpectra + operator-based, so solver is identical to explicit RSpectra
# 
# Inputs: 
#   output/tmp/geno_n_<n>.tsv
#   output/tmp/Phi_n_<n>_Amin.rds
#   scripts/Phi_prod.R   (defines Theta_prod)
# 
# Outputs:
#   output/tmp/rspectra_operator_theta_n_<n>_values.tsv
#   output/tmp/rspectra_operator_theta_n_<n>_vectors.tsv
#   output/tmp/rspectra_operator_theta_n_<n>_runtime.csv
#   output/tmp/rspectra_operator_theta_n_<n>_timing.csv
# ------------------------------------------------------------

suppressPackageStartupMessages({
  library(RSpectra)
})

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 1) stop("Usage: Rscript scripts/04f_run_rspectra_operator.R <n>")

n <- as.integer(args[1])
k <- min(10, n - 1)

geno_path <- sprintf("output/tmp/geno_n_%d.rds", n)
amin_path <- sprintf("output/tmp/Phi_n_%d_Amin.rds", n)

out_prefix <- sprintf("output/tmp/rspectra_operator_theta_n_%d", n)
rt_path <- paste0(out_prefix, "_runtime.csv")
time_path <- paste0(out_prefix, "_timing.csv")

dir.create("output/tmp", showWarnings = FALSE, recursive = TRUE)
stopifnot(file.exists(geno_path), file.exists(amin_path)) 

# Skip if outputs exist
if (file.exists(rt_path) && file.exists(time_path)) {
  cat("RSpectra-operator outputs exist, skipping:", rt_path, "\n")
  quit(save = "no", status = 0)
}

t_total0 <- Sys.time() 

# ---- Load X ---- 
t0 <- Sys.time()
X <- readRDS(geno_path) 
t_loadX <- as.numeric(difftime(Sys.time(), t0, units = "secs"))

n_check <- nrow(X)
if (n_check != n) stop("Loaded n = ", n_check, " but expected n = ", n)

# ---- Center X -> X1 ----
t0 <- Sys.time()
X1 <- sweep(X, 2, colMeans(X))
t_center <- as.numeric(difftime(Sys.time(), t0, units = "secs"))

# ---- Load A_min ----
t0 <- Sys.time()
A_min <- readRDS(amin_path)
t_loadAmin <- as.numeric(difftime(Sys.time(), t0, units = "secs"))

# ---- Build d ----
t0 <- Sys.time()
d <- rowMeans(X1^2 - 1) / A_min
t_buildd <- as.numeric(difftime(Sys.time(), t0, units = "secs"))

# ---- Load operator implementation ----
phi_prod_path <- file.path("scripts", "Phi_prod.R")
if (!file.exists(phi_prod_path)) stop("Missing: ", phi_prod_path)
source(phi_prod_path)

args_op <- list(X1 = X1, A_min = A_min, d = d)

# ---- Eigs using RSpectra with operator ----
t0 <- Sys.time()
fit <- RSpectra::eigs_sym(Theta_prod, k = k, n = n, which = "LA", args = args_op)
t_eigs <- as.numeric(difftime(Sys.time(), t0, units = "secs"))

# ---- Write outputs ----
t0 <- Sys.time()
write.table(fit$values, paste0(out_prefix, "_values.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(fit$vectors, paste0(out_prefix, "_vectors.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
t_write <- as.numeric(difftime(Sys.time(), t0, units = "secs"))

t_total <- as.numeric(difftime(Sys.time(), t_total0, units = "secs"))

# Keep runtime as eigs-only (consistent with other scripts)
write.csv(data.frame(runtime_sec = t_eigs), rt_path, row.names = FALSE)

# Save detailed timing
write.csv(
  data.frame(
    n = n,
    k = k,
    load_X_sec = t_loadX,
    center_X_sec = t_center,
    load_Amin_sec = t_loadAmin,
    build_d_sec = t_buildd,
    eigs_sec = t_eigs,
    write_outputs_sec = t_write,
    total_sec = t_total
  ),
  time_path,
  row.names = FALSE
)

cat("RSpectra-operator runtime (eigs-only, sec):", t_eigs, "\n")
cat("RSpectra-operator done.\n")
cat("Wrote:", time_path, "\n")


